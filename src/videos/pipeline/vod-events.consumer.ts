  ...
  private async processHighlightPayload(
    ...
    for (const h of highlights) {
      ...
      await this.clipModel.create({
        ...
        signals: {
          audio: !!(h.signals?.audio_spike || h.signals?.audio),
          scene: !!(h.signals?.scene_change || h.signals?.scene),
          chat: !!(h.signals?.chat_spike || h.signals?.chat),
          ocr: !!(h.signals?.ocr_keyword || h.signals?.ocr),
        },
